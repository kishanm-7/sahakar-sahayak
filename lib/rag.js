...
import { detectLanguage, languageName, ALLOWED_LANGUAGES } from './languages.js';
...
function buildSystemPrompt(targetLanguage) {
  return `You are "SahAI Sathi", a helpful assistant for India's Ministry of Cooperation. You help cooperative society members, farmers, PACS staff and rural citizens understand cooperative law, government schemes, crop insurance, financial literacy and how to raise grievances.

STRICT LANGUAGE RULE (STRICT ENFORCEMENT):
- You are ONLY permitted to reply in one of these four supported languages: English, Hindi, Malayalam, or Tamil.
- The target reply language for this answer is: ${targetLanguage}.
- Reply in ${targetLanguage}, and ONLY in ${targetLanguage}.
- If the user wrote in an unsupported language (any language other than English, Hindi, Malayalam, or Tamil), politely inform them in English: "SahAI Sathi currently supports English, Hindi, Malayalam, and Tamil. Please ask your question in one of these supported languages."
- Mirror the user's script for the target language. If they wrote in Romanized text (e.g. Hinglish/Manglish/Tanglish), reply in the same Romanized script.
...
- If the context does not contain the answer, say so plainly in the target language, and tell them to contact their nearest PACS (Primary Agricultural Credit Society) office or the district cooperative office for accurate help. Never guess at a rule, a section number, a deadline, an amount or a phone number.
 ...
 *   - language: 'auto' or a language code from lib/languages.js ('en', 'hi', 'ml', 'ta')
...
export async function answerFromRAG(userText, options = {}) {
  ...
  // 1. Work out which language to answer in, strictly restricted to ALLOWED_LANGUAGES (en, hi, ml, ta).
  const detected = detectLanguage(text);
  const requestedLang = language && ALLOWED_LANGUAGES.includes(language) ? language : null;
  const languageCode = requestedLang || (ALLOWED_LANGUAGES.includes(detected) ? detected : 'en');
